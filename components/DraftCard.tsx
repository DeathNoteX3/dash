...
const DraftCard: React.FC<DraftCardProps> = ({ draft, onEdit }) => {
    ...
    if (!draft.thumbnail || imgError) {
      return (
        <div className="w-full aspect-video bg-gray-200 dark:bg-yt-dark-gray flex items-center justify-center">
          <svg xmlns="http://www.w3.org/2000/svg" className="h-12 w-12 text-gray-400 dark:text-yt-text-secondary" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1}>
  ...
  return (
    <div 
        ...
        className="cursor-pointer group bg-white dark:bg-yt-dark-gray rounded-xl shadow-sm border border-gray-200/80 dark:border-yt-light-gray/60 transition-all duration-300 hover:shadow-lg hover:-translate-y-1 flex flex-col h-full overflow-hidden"
    >
      {renderThumbnail()}
