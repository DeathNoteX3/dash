...
const App: React.FC = () => {
    ...
    const loadData = async () => {
        try {
            if (window.electronAPI) {
                const storedTheme = await window.electronAPI.getStore('theme');
                const storedStages = await window.electronAPI.getStore('stagesConfig') as Stage[] | undefined;
                const storedDrafts = await window.electronAPI.getStore('video_drafts');
                const storedPublished = await window.electronAPI.getStore('published_videos');

                setTheme(storedTheme || 'dark');
                const loadedStages = storedStages || initialStagesConfig;
                setStagesConfig(loadedStages);

                const allTemplateTasks = loadedStages.flatMap(stage => stage.tasks);
                const syncChecklist = (video: Video): Video => ({
                    ...video,
                    checklist: allTemplateTasks.map(templateTask => {
                    ...
                    postPublicationChecklist: video.postPublicationChecklist?.length ? video.postPublicationChecklist : initialPostPublicationChecklist,
                });

                setDrafts((storedDrafts || []).map(syncChecklist));
                setPublishedVideos((storedPublished || []).map(syncChecklist));
            } else {
  ...
  useEffect(() => {
    if (!isLoading && window.electronAPI) {
      window.electronAPI.setStore('video_drafts', drafts);
    }
  }, [drafts, isLoading]);

  useEffect(() => {
    if (!isLoading && window.electronAPI) {
      window.electronAPI.setStore('published_videos', publishedVideos);
    }
  }, [publishedVideos, isLoading]);

  useEffect(() => {
    if (!isLoading && window.electronAPI) {
      window.electronAPI.setStore('stagesConfig', stagesConfig);
    }
  }, [stagesConfig, isLoading]);
  ...
  const allTemplateTasks = useMemo(() => stagesConfig.flatMap(stage => stage.tasks), [stagesConfig]);
  ...
  const handleSaveDraft = (draftData: Video) => {
    const isNew = !draftData.id;
    if (isNew) {
      const newDraft = { ...draftData, id: `video_${Date.now()}` };
      setDrafts(prev => [newDraft, ...prev]);
      showToast('Rascunho adicionado com sucesso!');
    } else {
      const isPublishedVideo = publishedVideos.some(v => v.id === draftData.id);
      if (isPublishedVideo) {
        setPublishedVideos(prev => prev.map(v => (v.id === draftData.id ? draftData : v)));
        showToast('Vídeo atualizado com sucesso!');
      } else {
        setDrafts(prev => prev.map(d => (d.id === draftData.id ? draftData : d)));
  ...
  const handleUpdatePostPublicationChecklist = (videoId: string, newChecklist: PostPublicationChecklistItem[]) => {
    setPublishedVideos(prev =>
        prev.map(v => (v.id === videoId ? { ...v, postPublicationChecklist: newChecklist } : v))
    );
  };
  ...
  const handleExportBackup = () => {
    try {
        const data = JSON.stringify({ drafts, publishedVideos, stagesConfig }, null, 2);
        const blob = new Blob([data], { type: 'application/json' });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = `videodash_backup_${new Date().toISOString().split('T')[0]}.json`;
  ...
  const handleFileSelected = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
        try {
            ...
            const { drafts: importedDrafts, publishedVideos: importedPublished, stagesConfig: importedStagesConfig } = JSON.parse(result);
  ...
  const nextVideoNumber = useMemo(() => {
    const allVideos = [...drafts, ...publishedVideos];
    if (allVideos.length === 0) return 1;

    const maxNumber = allVideos.reduce((max, video) => {
        return (video.videoNumber && video.videoNumber > max) ? video.videoNumber : max;
    }, 0);
    return maxNumber + 1;
  }, [drafts, publishedVideos]);
  ...
  if (isLoading) {
    return (
        <div className="h-screen bg-white dark:bg-yt-black flex items-center justify-center">
            <div className="flex items-center gap-4">
                ...
                <span className="text-xl font-semibold text-gray-800 dark:text-white">Carregando...</span>
            </div>
        </div>
    );
  }
  ...
  return (
    <div className="h-screen overflow-hidden text-gray-800 dark:text-yt-text-primary font-sans">
        <Toast message={toast.message} isVisible={toast.isVisible} />
        <div className="bg-white dark:bg-yt-black flex w-full h-full rounded-lg overflow-hidden border border-gray-400/20 dark:border-yt-light-gray/30">
            ...
            <div className="flex-1 flex flex-col">
                <header className="py-2 px-8 flex justify-between items-center border-b border-gray-200/80 dark:border-yt-light-gray/50 flex-shrink-0">
